...
diff_lat <- abs(lat_m[len]-lat_m[1])
diff_lon <- abs(lon_m[len]-lon_m[1])
sum <- diff_lat + diff_lon
...
factor_lat <- diff_lat/sum
factor_lon <- diff_lon/sum
...
P_n_n_prev <-  rbind(
  ...
  c(0,p_factor_error * factor_lon,0,0),
  c(0,0,factor_lon * p_factor_error * mean(delta_t,na.rm=TRUE),0),
  c(0,0,0,factor_lon * p_factor_error * mean(delta_t,na.rm=TRUE))
)
...
#r <- ((1 / ( speed[1]-1.4)) * 5.33) + 6.33
#r <- ((1 / ( 1.5-1.3)) * 5.33) + 6.33
r <- 90 - ((100/3) *speed[1])

if (r <- 9) {
  r <- 9
}
r_factor_error <- p_factor_error * r
R <- rbind(
  c(r_factor_error * factor_lon,0,0,0),
  c(0,r_factor_error * factor_lon,0,0),
  c(0,0,factor_lon * r_factor_error * mean(delta_t,na.rm=TRUE),0),
  c(0,0,0,factor_lon * r_factor_error * mean(delta_t,na.rm=TRUE))
)     
...
print(paste("diff_x: " ,diff_x,sep=""))
print(paste("diff_y: " ,diff_y,sep=""))
...
### end loop
  ...
  diff_lat <- max(lat_m) - min(lat_m)
  diff_lon <- max(lon_m) - min(lon_m)
  ...
  plot(lon_m,lat_m, 
       ylim = c(mean_lat-diff/2, mean_lat+diff/2),
       xlim = c(mean_lon-diff/2, mean_lon+diff/2)
       )
  par(new=TRUE)
  ...
  plot(speed, lwd=2, col="black", ylim=c(min,max))
